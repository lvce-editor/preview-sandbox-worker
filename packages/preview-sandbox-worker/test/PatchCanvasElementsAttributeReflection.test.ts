...
test('patchCanvasElements should preserve id after getContext and dimension updates', async () => {
  if (!('OffscreenCanvas' in globalThis)) {
    // @ts-ignore
    globalThis.OffscreenCanvas = MockOffscreenCanvas
  }
  ...
  const window = new Window({ url: 'https://localhost:3000' })
  const { document } = window
  document.documentElement.innerHTML = '<body><canvas id="gameCanvas" width="200" height="150"></canvas></body>'
  const mockOffscreenCanvas = new MockOffscreenCanvas(200, 150)
  ...
  await PatchCanvasElements.patchCanvasElements(document, 1)
  const canvas = document.querySelector('#gameCanvas') as any

  canvas.getContext('2d')
  canvas.width = 240

  expect(canvas.id).toBe('gameCanvas')
  expect(canvas.getAttribute('id')).toBe('gameCanvas')
